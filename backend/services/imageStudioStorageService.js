...
export class ImageStudioStorageService {
  async storeProcessedImage({ sourceUrl, userId, operation, contentType = 'image/webp' }) {
    ...
    try {
      ...
      if (!response.ok) throw new Error(`Téléchargement du résultat impossible (${response.status}).`);
      ...
      // The processed provider asset is already durable. A secondary-storage outage
      // must not discard an otherwise successful seller edit.
      console.warn('[image-studio] Firebase handoff failed; using managed storage.', error?.message);
      return { url: sourceUrl, storage: 'managed-provider', storageFallback: true };
    }
  }
}
