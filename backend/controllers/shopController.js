...
export const getShopProfile = asyncHandler(async (req, res) => {
  ...
  const [totalProducts, productsRaw] = await Promise.all([
    ...
    Product.find(publicProductFilter)
      .select(
        '_id title price images attributes category condition city createdAt slug salesCount favoritesCount whatsappClicks views discount priceBeforeDiscount certified installmentEnabled installmentStartDate installmentEndDate wholesaleEnabled wholesaleTiers quotationEnabled'
      )
      .sort('-createdAt')
      .skip(skip)
      .limit(limit)
