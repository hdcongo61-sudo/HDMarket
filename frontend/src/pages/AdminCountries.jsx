...
import { ArrowLeftIcon, ArrowRightIcon, CheckIcon, PlusIcon, RocketLaunchIcon, XMarkIcon } from '@heroicons/react/24/outline';
  ...
  return (
    <div className="mx-auto w-full max-w-7xl px-4 py-5 sm:px-6 lg:px-8">
      <div className="flex flex-wrap items-end justify-between gap-4">
        ...
        <button onClick={() => { setWizard(true); setStep(0); setForm(initialForm); }} className="inline-flex min-h-12 items-center gap-2 rounded-2xl bg-[#e85d00] px-5 font-black text-white"><PlusIcon className="h-[18px] w-[18px]" /> Ajouter un pays</button>
      </div>
      ...
      {wizard ? <div className="fixed inset-0 z-[180] flex items-end justify-center bg-black/50 sm:items-center sm:p-6"><section className="max-h-[94dvh] w-full max-w-2xl overflow-y-auto rounded-t-[30px] bg-[#f8f5f0] p-5 sm:rounded-[30px] sm:p-7">
        <div className="flex items-center justify-between"><div><p className="text-xs font-black uppercase tracking-widest text-[#e85d00]">Étape {step + 1} sur 8</p><h2 className="mt-1 text-2xl font-black">{STEPS[step]}</h2></div><button onClick={() => setWizard(false)} className="grid h-10 w-10 place-items-center rounded-full bg-white"><XMarkIcon className="h-[18px] w-[18px]" /></button></div>
