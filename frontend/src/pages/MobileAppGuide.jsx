...
import { ArrowDownTrayIcon, ArrowPathIcon, ArrowTopRightOnSquareIcon, CheckCircleIcon, CheckIcon, ChevronRightIcon, DevicePhoneMobileIcon, EllipsisVerticalIcon, HomeIcon, ShareIcon, ShieldCheckIcon, ShoppingBagIcon } from '@heroicons/react/24/outline';
    ...
    <div className="min-h-screen bg-[#fffaf5] text-[#221f1b]">
      <section className="border-b border-black/10 bg-[#201f1d] px-4 py-12 text-white sm:px-6 lg:py-16">
        <div className="mx-auto max-w-6xl">
          <div>
            <span className="inline-flex items-center gap-2 rounded-full border border-white/15 bg-white/10 px-3 py-2 text-xs font-black uppercase tracking-[.12em]"><DevicePhoneMobileIcon className="h-4 w-4 text-[#ff7a22]" />HDMarket sur votre téléphone</span>
            ...
            <div className="mt-7 flex flex-col gap-3 sm:flex-row">
              {installed ? <span className="inline-flex min-h-12 items-center justify-center gap-2 rounded-2xl bg-emerald-500 px-5 text-sm font-black"><CheckCircleIcon className="h-5 w-5" />Application déjà installée</span> : canInstall ? <button type="button" onClick={installAndroid} className="inline-flex min-h-12 items-center justify-center gap-2 rounded-2xl bg-[#e85d00] px-5 text-sm font-black text-white shadow-sm"><ArrowDownTrayIcon className="h-5 w-5" />Installer HDMarket</button> : <button type="button" onClick={() => scrollToGuide(platform === 'ios' ? 'ios' : 'android')} className="inline-flex min-h-12 items-center justify-center gap-2 rounded-2xl bg-[#e85d00] px-5 text-sm font-black text-white shadow-sm"><ChevronRightIcon className="h-5 w-5" />Voir les étapes</button>}
      ...
      <div className="mx-auto max-w-6xl px-4 py-10 sm:px-6 lg:py-16">
        ...
        <section className="mt-12 grid gap-6 lg:grid-cols-2">
          <article className="rounded-2xl border border-amber-200 bg-amber-50 p-6"><ArrowPathIcon className="h-7 w-7 text-amber-700" /><h2 className="mt-4 text-xl font-black">L’option d’installation n’apparaît pas ?</h2><ul className="mt-4 space-y-3 text-sm leading-6 text-amber-950"><li>• Vérifiez que vous utilisez Safari sur iPhone ou Chrome sur Android.</li><li>• Actualisez www.hdmarket.store, puis ouvrez de nouveau le menu.</li><li>• Si HDMarket est déjà installé, recherchez son icône sur l’écran d’accueil.</li><li>• Désactivez temporairement le mode navigation privée.</li></ul><div className="mt-5 flex flex-wrap gap-2"><a href="https://support.apple.com/guide/iphone/iphea86e5236/ios" target="_blank" rel="noreferrer" className="inline-flex min-h-10 items-center gap-1.5 rounded-xl bg-white px-3 text-xs font-black text-amber-950">Guide officiel Apple<ArrowTopRightOnSquareIcon className="h-3.5 w-3.5" /></a><a href="https://support.google.com/chrome/answer/9658361?co=GENIE.Platform%3DAndroid" target="_blank" rel="noreferrer" className="inline-flex min-h-10 items-center gap-1.5 rounded-xl bg-white px-3 text-xs font-black text-amber-950">Guide officiel Google<ArrowTopRightOnSquareIcon className="h-3.5 w-3.5" /></a></div></article>
          <article className="rounded-2xl bg-[#201f1d] p-6 text-white"><ShieldCheckIcon className="h-7 w-7 text-[#ff7a22]" /><h2 className="mt-4 text-xl font-black">Installation sûre et légère</h2><p className="mt-3 text-sm leading-6 text-white/65">L’application installée utilise directement www.hdmarket.store. Elle ne demande aucun téléchargement provenant d’un site tiers.</p><a href="https://www.hdmarket.store" className="mt-5 inline-flex min-h-11 items-center gap-2 rounded-xl bg-white px-4 text-sm font-black text-[#201f1d]">Ouvrir HDMarket<ArrowTopRightOnSquareIcon className="h-4 w-4" /></a></article>
        </section>
      </div>
    </div>
  );
...
function StepVisual({ platform, step }) {
  ...
  return <div className="relative flex h-52 items-end justify-center overflow-hidden border-b border-[#ddd5cc] bg-[#f1ece6] px-8 pt-6"><div className="absolute left-4 top-4 rounded-lg bg-white px-2.5 py-1 text-[10px] font-black uppercase tracking-wider text-stone-500">{isIos ? 'Safari' : 'Chrome'}</div><div className="relative h-48 w-32 rounded-2xl border-[5px] border-[#282522] bg-white"><div className="mx-auto mt-1.5 h-2 w-12 rounded-full bg-[#282522]" /><div className="mx-2 mt-3 rounded-lg bg-[#fff3e8] p-2"><div className="flex items-center gap-1"><ShoppingBagIcon className="h-3.5 w-3.5 text-[#e85d00]" /><span className="text-[7px] font-black">HDMarket</span></div><div className="mt-2 h-10 rounded-md bg-stone-200" /></div>{step === 0 ? <div className="absolute inset-x-2 bottom-3 rounded-lg border border-stone-200 bg-white p-2 text-center text-[7px] font-bold">www.hdmarket.store</div> : null}{step === 1 ? <div className="absolute inset-x-2 bottom-3 flex items-center justify-center rounded-lg bg-[#201f1d] py-3 text-white">{isIos ? <ShareIcon className="h-5 w-5" /> : <EllipsisVerticalIcon className="h-5 w-5" />}</div> : null}{step === 2 ? <div className="absolute inset-x-1 bottom-1 rounded-lg border border-stone-200 bg-white p-2"><div className="flex items-center gap-2 rounded-md bg-[#fff0e4] p-2"><HomeIcon className="h-4 w-4 text-[#e85d00]" /><span className="text-[7px] font-black">{isIos ? 'Sur l’écran d’accueil' : 'Ajouter à l’écran d’accueil'}</span></div></div> : null}{isIos && step === 3 ? <div className="absolute inset-x-2 bottom-4 flex items-center justify-between rounded-lg border border-stone-200 bg-white p-2 text-[7px] font-black"><span>Ouvrir comme app</span><span className="h-4 w-7 rounded-full bg-emerald-500 p-0.5"><span className="ml-auto block h-3 w-3 rounded-full bg-white" /></span></div> : null}{((isIos && step === 4) || (!isIos && step === 3)) ? <div className="absolute inset-x-2 bottom-4 rounded-lg bg-[#e85d00] p-2 text-center text-[8px] font-black text-white"><CheckIcon className="mr-1 inline h-3 w-3" />{isIos ? 'Ajouter' : 'Installer'}</div> : null}</div></div>;
}
