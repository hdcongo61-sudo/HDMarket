...
function Stat({ icon: Icon, label, value }) {
  return <div className="rounded-2xl border border-neutral-200 bg-white p-4 dark:border-white/10 dark:bg-neutral-900"><div className="flex items-center gap-2 text-xs font-semibold text-neutral-500"><Icon size={15} />{label}</div><p className="mt-2 text-2xl font-black">{Number(value || 0).toLocaleString('fr-FR')}</p></div>;
}
...
export default function AdminProductVideos() {
  ...
  return (
    <div className="space-y-7 p-4 sm:p-6 lg:p-8">
      <div className="flex flex-wrap items-end justify-between gap-4"><div><p className="text-sm font-bold uppercase tracking-[0.18em] text-violet-600">Confiance & croissance</p><h1 className="text-3xl font-black">HDMarket Videos</h1><p className="mt-1 text-sm text-neutral-500">Modération, mise en avant, sponsoring et conversion du flux achetable.</p></div><button type="button" onClick={load} className="flex h-10 items-center gap-2 rounded-xl border border-neutral-200 px-4 text-sm font-bold dark:border-white/10"><RefreshCw size={16} /> Actualiser</button></div>

      <div className="grid grid-cols-2 gap-3 md:grid-cols-4 xl:grid-cols-8"><Stat icon={Play} label="Vidéos" value={analytics.videos} /><Stat icon={ShieldAlert} label="En attente" value={analytics.pending} /><Stat icon={Eye} label="Vues" value={analytics.views} /><Stat icon={BarChart3} label="Complétions" value={analytics.completions} /><Stat icon={MousePointerClick} label="Clics produit" value={analytics.productClicks} /><Stat icon={ShoppingCart} label="Ajouts panier" value={analytics.addToCarts} /><Stat icon={Megaphone} label="Sponsorisé" value={analytics.sponsored} /><Stat icon={Flag} label="Signalements" value={analytics.reports} /></div>
      ...
      {tab === 'videos' ? <>
        ...
        <div className="grid gap-4 xl:grid-cols-2">{items.map((video) => <article key={video._id} className="grid overflow-hidden rounded-2xl border border-neutral-200 bg-white sm:grid-cols-[180px_1fr] dark:border-white/10 dark:bg-neutral-900"><div className="relative aspect-[4/5] bg-neutral-950 sm:aspect-auto"><img src={video.thumbnailUrl || video.product?.images?.[0]} alt="" className="h-full w-full object-cover" loading="lazy" />{video.sponsored ? <span className="absolute left-2 top-2 rounded-full bg-violet-600 px-2 py-1 text-[10px] font-bold text-white">Sponsorisé</span> : null}{video.featured ? <BadgeCheck className="absolute right-2 top-2 text-amber-300" fill="currentColor" /> : null}</div><div className="flex min-w-0 flex-col p-4"><div className="flex items-start justify-between gap-3"><div><p className="text-xs font-bold uppercase text-neutral-500">{video.seller?.shopName || video.seller?.name}</p><h2 className="mt-1 line-clamp-2 font-black">{video.product?.title || 'Produit supprimé'}</h2></div><span className="rounded-full bg-neutral-100 px-2.5 py-1 text-[10px] font-bold uppercase dark:bg-white/10">{LABELS[video.status] || video.status}</span></div><p className="mt-3 line-clamp-3 text-sm text-neutral-600 dark:text-neutral-300">{video.caption || 'Sans légende'}</p><div className="mt-3 flex flex-wrap gap-3 text-xs text-neutral-500"><span>{video.counters?.views || 0} vues</span><span>{video.counters?.likes || 0} J’aime</span><span>{video.counters?.productClicks || 0} clics</span></div>{video.moderationReason ? <p className="mt-3 rounded-lg bg-rose-50 p-2 text-xs text-rose-700 dark:bg-rose-950/30">{video.moderationReason}</p> : null}<div className="mt-auto flex flex-wrap gap-2 pt-4">{video.status !== 'approved' ? <button type="button" disabled={busyId === video._id} onClick={() => moderate(video, 'approve')} className="flex h-9 items-center gap-1 rounded-lg bg-emerald-500 px-3 text-xs font-bold text-white"><Check size={14} /> Approuver</button> : <button type="button" onClick={() => moderate(video, 'hide', 'Masquée par la modération')} className="flex h-9 items-center gap-1 rounded-lg border border-neutral-200 px-3 text-xs font-bold dark:border-white/10"><X size={14} /> Masquer</button>}<button type="button" onClick={() => moderate(video, video.featured ? 'unfeature' : 'feature')} className="h-9 rounded-lg border border-amber-200 px-3 text-xs font-bold text-amber-700">{video.featured ? 'Retirer sélection' : 'Mettre en avant'}</button><button type="button" onClick={() => moderate(video, video.sponsored ? 'unsponsor' : 'sponsor')} className="h-9 rounded-lg border border-violet-200 px-3 text-xs font-bold text-violet-700">{video.sponsored ? 'Retirer sponsor' : 'Sponsoriser'}</button>{video.status === 'pending' ? <button type="button" onClick={() => moderate(video, 'reject', 'Contenu non conforme aux règles vidéo HDMarket')} className="h-9 rounded-lg border border-rose-200 px-3 text-xs font-bold text-rose-600">Refuser</button> : null}<button type="button" title="Suspendre le vendeur" onClick={() => moderate(video, 'ban_seller', 'Contenu vidéo non conforme')} className="grid h-9 w-9 place-items-center rounded-lg border border-rose-200 text-rose-600"><UserX size={15} /></button><button type="button" title="Supprimer" onClick={() => moderate(video, 'delete', 'Suppression administrative')} className="grid h-9 w-9 place-items-center rounded-lg border border-neutral-200 text-neutral-500 dark:border-white/10"><Trash2 size={15} /></button></div></div></article>)}</div>
