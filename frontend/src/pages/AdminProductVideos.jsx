...
export default function AdminProductVideos() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [videosResult, analyticsResult, reportsResult] = await Promise.all([
        api.get('/product-videos/admin', { params: { status: filter || undefined, limit: 50, page }, headers: { 'x-skip-cache': '1' } }),
      ...
      setPages(Math.max(1, videosResult.data?.pages || 1));
  ...
  return (
    <div className="space-y-7 p-4 sm:p-6 lg:p-8">
      ...
      {tab === 'videos' ? <>
        <div className="flex flex-wrap gap-2">{FILTERS.map((value) => <button key={value || 'all'} type="button" onClick={() => { setPage(1); setFilter(value); }} className={`rounded-full px-4 py-2 text-xs font-bold ${filter === value ? 'bg-neutral-950 text-white dark:bg-white dark:text-neutral-950' : 'bg-neutral-100 text-neutral-600 dark:bg-white/10 dark:text-neutral-300'}`}>{LABELS[value]}</button>)}</div>
