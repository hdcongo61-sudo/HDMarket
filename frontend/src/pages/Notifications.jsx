...
export default function Notifications() {
  ...
  const typeConfig = {
    product_comment: { label: 'Commentaire', badgeClass: 'bg-green-100 text-green-700' },
    reply: { label: 'Réponse', badgeClass: 'bg-blue-100 text-blue-700' },
    favorite: { label: 'Favori', badgeClass: 'bg-pink-100 text-pink-700' },
    rating: { label: 'Note', badgeClass: 'bg-amber-100 text-amber-700' }
  };
  ...
  const renderAlert = (alert, isUnread) => {
    ...
    return (
      ...
      >
        {(() => {
          const info = typeConfig[alert.type] || {
            label: 'Notification',
          ...
          return (
            <div className="flex items-center gap-2 text-xs">
              ...
              <span className={`inline-flex items-center px-2 py-0.5 rounded-full font-semibold ${info.badgeClass}`}>
                {info.label}
              </span>
        ...
        {alert.user?.name ? (
          <div className="text-xs text-gray-500">
            <span className="font-medium text-gray-700">{alert.user.name}</span>
          </div>
        ) : null}
        ...
        {alert.product &&
          (canViewListing ? (
            ...
            <span className="inline-flex items-center gap-1 text-sm text-gray-500 italic">
              Annonce en attente de validation : {alert.product.title}
            </span>
          ))}
      </li>
    );
    ...
    <main className="max-w-4xl mx-auto p-4 space-y-6">
      <header className="space-y-1">
        ...
        <p className="text-sm text-gray-500">
          Suivez les dernières interactions laissées sur vos annonces (commentaires, réponses, favoris, notes).
        </p>
      </header>
