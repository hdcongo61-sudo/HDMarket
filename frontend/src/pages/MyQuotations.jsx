...
import { ArrowLeft, CalendarClock, Check, ChevronRight, FileText, MessageCircle, Package, Store, X } from 'lucide-react';
    ...
    <main className="min-h-screen bg-[#f6f3ee] px-3 pb-28 pt-4 text-[#231f1b] sm:px-6 lg:py-10 dark:bg-neutral-950 dark:text-white">
      <div className="mx-auto max-w-5xl">
        <header className="flex items-start gap-3">
          <Link to="/orders" className="grid h-11 w-11 shrink-0 place-items-center rounded-full border border-[#ded6ca] bg-white dark:border-neutral-800 dark:bg-neutral-900"><ArrowLeft className="h-5 w-5" /></Link>
        ...
        {!loading && !rows.length ? (
          <section className="mt-8 rounded-3xl border border-[#e2dcd2] bg-white px-6 py-14 text-center dark:border-neutral-800 dark:bg-neutral-900">
            <span className="mx-auto grid h-16 w-16 place-items-center rounded-2xl bg-[#fff0e4] text-[#e85d00]"><FileText className="h-7 w-7" /></span>
            ...
            return (
              <article id={`quotation-${quotation._id}`} key={quotation._id} className={`overflow-hidden rounded-3xl border bg-white shadow-sm dark:bg-neutral-900 ${quotationId === quotation._id ? 'border-[#e85d00] ring-2 ring-[#e85d00]/10' : 'border-[#e2dcd2] dark:border-neutral-800'}`}>
                <div className="flex items-center gap-3 border-b border-[#eee8df] p-4 dark:border-neutral-800">
                  <span className="grid h-11 w-11 place-items-center rounded-2xl bg-[#fff0e4] text-[#e85d00]"><Store className="h-5 w-5" /></span>
                ...
                <div className="p-4">
                  <div className="space-y-2">
                    {(quotation.items || []).map((item) => <div key={item._id} className="flex items-center gap-3 rounded-2xl bg-[#f8f5f0] p-2.5 dark:bg-neutral-950">
                      <div className="h-12 w-12 shrink-0 overflow-hidden rounded-xl bg-[#eee8df]">{(item.snapshot?.image || item.product?.images?.[0]) ? <img src={item.snapshot?.image || item.product?.images?.[0]} alt="" className="h-full w-full object-cover" /> : <Package className="m-3 h-6 w-6 text-[#aaa298]" />}</div>
                  ...
                  <div className="mt-4 grid grid-cols-2 gap-3 rounded-2xl border border-[#eee8df] p-3 sm:grid-cols-4 dark:border-neutral-800">
                    ...
                    <div><p className="text-[10px] font-black uppercase text-[#8a8378]">Expiration</p><p className="mt-1 flex items-center gap-1 text-xs font-black"><CalendarClock className="h-3.5 w-3.5" />{formatQuotationDate(quotation.expirationDate, true)}</p></div>
                  </div>
                  ...
                  <div className="mt-4 flex flex-wrap gap-2">
                    {quotation.status === 'COUNTERED' ? <button disabled={busy} onClick={() => mutate(quotation, 'accept')} className="inline-flex min-h-11 flex-1 items-center justify-center gap-2 rounded-xl bg-[#e85d00] px-4 text-sm font-black text-white"><Check className="h-4 w-4" />Accepter l’offre</button> : null}
                    {quotation.status === 'ACCEPTED' ? <button onClick={() => navigate(`/quotations/${quotation._id}/checkout`)} className="inline-flex min-h-11 flex-1 items-center justify-center gap-2 rounded-xl bg-[#e85d00] px-4 text-sm font-black text-white">Finaliser la commande <ChevronRight className="h-4 w-4" /></button> : null}
                    {quotation.status === 'ORDER_CREATED' && quotation.order?._id ? <Link to={`/orders/detail/${quotation.order._id}`} className="inline-flex min-h-11 flex-1 items-center justify-center gap-2 rounded-xl bg-[#231f1b] px-4 text-sm font-black text-white">Voir la commande <ChevronRight className="h-4 w-4" /></Link> : null}
                    {['COUNTERED', 'ACCEPTED'].includes(quotation.status) ? <button disabled={busy} onClick={() => mutate(quotation, 'reject')} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl border border-[#ded6ca] px-4 text-sm font-black"><X className="h-4 w-4" />Refuser</button> : null}
                    <button onClick={() => messageSeller(quotation)} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl border border-[#ded6ca] px-4 text-sm font-black"><MessageCircle className="h-4 w-4" />Message</button>
                  </div>
                </div>
              </article>
            );
