...
import { ArrowPathIcon, CalendarDaysIcon, CheckIcon, CubeIcon, DocumentTextIcon, MapPinIcon, PaperAirplaneIcon, ReceiptPercentIcon, UserCircleIcon, XMarkIcon } from '@heroicons/react/24/outline';
  ...
  return (
    <main className="min-h-screen bg-[#f5f2ee] px-3 pb-28 pt-5 text-[#231f1b] sm:px-6 lg:px-8 lg:py-8 dark:bg-neutral-950 dark:text-white">
      <div className="mx-auto max-w-6xl">
        ...
        {!loading && !rows.length ? <section className="mt-8 rounded-3xl border border-[#e2dcd2] bg-white py-16 text-center dark:border-neutral-800 dark:bg-neutral-900"><DocumentTextIcon className="mx-auto h-10 w-10 text-[#e85d00]" /><h2 className="mt-4 text-lg font-black">Aucune demande dans cet onglet</h2></section> : null}
        <section className="mt-5 grid items-start gap-4 lg:grid-cols-2">{rows.map((quotation) => {
          ...
          return <article key={quotation._id} className={`overflow-hidden rounded-3xl border bg-white shadow-sm dark:bg-neutral-900 ${selected?._id === quotation._id ? 'border-[#e85d00] ring-2 ring-[#e85d00]/10' : 'border-[#e2dcd2] dark:border-neutral-800'}`}>
            ...
            <div className="p-4"><div className="space-y-2">{(quotation.items || []).map((item) => <div key={item._id} className="flex items-center gap-3 rounded-2xl bg-[#f8f5f0] p-2.5 dark:bg-neutral-950"><div className="h-12 w-12 overflow-hidden rounded-xl bg-[#eee8df]">{(item.snapshot?.image || item.product?.images?.[0]) ? <img src={item.snapshot?.image || item.product?.images?.[0]} alt="" className="h-full w-full object-cover" /> : <CubeIcon className="m-3 h-6 w-6" />}</div><div className="min-w-0 flex-1"><p className="truncate text-sm font-black">{item.snapshot?.title || item.product?.title}</p><p className="text-xs text-[#797166]">Qté {item.quantity} · souhaité {item.requestedPrice ? quotationMoney(item.requestedPrice) : 'non précisé'}</p><SelectedAttributesList selectedAttributes={item.selectedAttributes} compact className="mt-1" /></div><strong className="text-xs">{quotationMoney(item.originalPrice)}</strong></div>)}</div>
              ...
              {canRespond ? <div className="mt-4 grid grid-cols-3 gap-2"><button disabled={working} onClick={() => respond(quotation, 'ACCEPT', { validityHours: 48 })} className="inline-flex min-h-11 items-center justify-center gap-1 rounded-xl bg-emerald-600 px-2 text-xs font-black text-white"><CheckIcon className="h-4 w-4" />Accepter</button><button disabled={working} onClick={() => openCounter(quotation)} className="inline-flex min-h-11 items-center justify-center gap-1 rounded-xl bg-[#e85d00] px-2 text-xs font-black text-white"><PaperAirplaneIcon className="h-4 w-4" />Contre-offre</button><button disabled={working} onClick={() => respond(quotation, 'REJECT', {})} className="inline-flex min-h-11 items-center justify-center gap-1 rounded-xl border border-[#ded6ca] px-2 text-xs font-black"><XMarkIcon className="h-4 w-4" />Refuser</button></div> : null}
            </div>
          </article>;
        })}</section>
      </div>

      {offerOpen && selected ? <div className="fixed inset-0 z-[100] flex items-end justify-center bg-black/55 p-0 sm:items-center sm:p-4" role="dialog" aria-modal="true"><form onSubmit={submitCounter} className="max-h-[92dvh] w-full overflow-y-auto rounded-t-3xl bg-[#f8f5f0] p-4 shadow-2xl sm:max-w-2xl sm:rounded-3xl sm:p-6 dark:bg-neutral-950"><div className="flex items-start justify-between gap-3"><div><p className="text-xs font-black uppercase tracking-[.15em] text-[#e85d00]">Proposition vendeur</p><h2 className="text-2xl font-black">Créer une contre-offre</h2></div><button type="button" onClick={() => setOfferOpen(false)} className="grid h-10 w-10 place-items-center rounded-full bg-white dark:bg-neutral-900"><XMarkIcon className="h-5 w-5" /></button></div>
