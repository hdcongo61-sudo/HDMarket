...
import { ArrowPathIcon, ArrowTrendingUpIcon, BuildingStorefrontIcon, ChartBarIcon, CubeIcon, DocumentTextIcon, ReceiptPercentIcon } from '@heroicons/react/24/outline';
...
export default function AdminQuotations() {
  ...
  const load = useCallback(async () => {
    ...
    } catch (requestError) { setError(requestError.response?.data?.message || 'Impossible de charger la gestion des prix à débattre.'); }
    finally { setLoading(false); }
  }, [filter]);
  useEffect(() => { load(); }, [load]);
  const metrics = [
    ['Demandes', analytics?.requests || 0, DocumentTextIcon],
    ['Conversion', `${Number(analytics?.conversionRate || 0).toFixed(1)} %`, ArrowTrendingUpIcon],
    ['Remise moyenne', `${Number(analytics?.averageDiscount || 0).toFixed(1)} %`, ReceiptPercentIcon],
    ['Revenu prix à débattre', quotationMoney(analytics?.revenue || 0), ChartBarIcon]
  ];
  return <main className="min-h-screen bg-[#f5f2ee] p-4 text-[#231f1b] sm:p-6 lg:p-8 dark:bg-neutral-950 dark:text-white"><div className="mx-auto max-w-7xl">
    <header className="flex items-end justify-between gap-4"><div><p className="text-xs font-black uppercase tracking-[.16em] text-[#e85d00]">Administration</p><h1 className="mt-1 text-2xl font-black sm:text-4xl">Gestion des prix à débattre</h1><p className="mt-1 text-sm text-[#797166]">Performance, négociations et commandes générées.</p></div><button onClick={load} className="grid h-11 w-11 place-items-center rounded-full border border-[#ded6ca] bg-white dark:border-neutral-800 dark:bg-neutral-900" aria-label="Actualiser"><ArrowPathIcon className="h-4 w-4" /></button></header>
