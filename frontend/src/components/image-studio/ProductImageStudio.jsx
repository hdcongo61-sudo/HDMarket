...
export default function ProductImageStudio({ isOpen, image, images = [], initialIndex = 0, shopName = '', onClose, onSave }) {
  ...
  const renderPanel = () => {
    ...
    return <div className="space-y-5"><PanelTitle title="Optimisation & export" subtitle="WEBP offre le meilleur équilibre pour HDMarket." /><SelectField label="Format" value={state.output.format} options={[['image/webp','WEBP — recommandé'],['image/jpeg','JPEG'],['image/png','PNG'],['image/avif','AVIF']]} onChange={(format) => change((prev) => ({ ...prev, output: { ...prev.output, format } }))} /><SelectField label="Compression" value={state.output.compression} options={[['low','Faible — qualité maximale'],['medium','Moyenne — recommandée'],['high','Élevée — fichier léger']]} onChange={(compression) => change((prev) => ({ ...prev, output: { ...prev.output, compression } }))} /><div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-4"><span className="block text-xs font-bold text-emerald-700">Taille finale estimée</span><span className="mt-1 block text-2xl font-black text-emerald-950">{estimate ? formatFileSize(estimate) : 'Calcul…'}</span></div><QualityPanel quality={quality} /><button type="button" onClick={handleSave} disabled={!loadedImage || saving} className="flex min-h-12 w-full items-center justify-center gap-2 rounded-2xl bg-[#231f1b] px-4 text-sm font-black text-white shadow-lg transition hover:bg-[#3a342f] disabled:cursor-not-allowed disabled:opacity-50">{saving ? <Loader2 className="h-5 w-5 animate-spin" /> : <Save className="h-5 w-5" />}{saving ? 'Enregistrement…' : 'Enregistrer la photo'}</button></div>;
  };
  ...
  return <div className="fixed inset-0 z-[1000] flex flex-col overflow-hidden bg-[#201f1d] text-[#231f1b] lg:bg-[#f5f2ee]" role="dialog" aria-modal="true" aria-label="Studio photo produit">
    <header className="relative z-40 flex min-h-14 items-center gap-1.5 border-b border-white/10 bg-[#201f1d] px-2 pt-[env(safe-area-inset-top)] text-white lg:min-h-16 lg:gap-2 lg:border-[#e2dcd2] lg:bg-white lg:px-5 lg:text-[#231f1b]">
      ...
      <button type="button" onClick={handleSave} disabled={!loadedImage || saving} className="flex min-h-11 items-center gap-2 rounded-xl bg-[#e85d00] px-3 text-sm font-black text-white shadow-lg disabled:opacity-50 lg:rounded-full lg:bg-[#231f1b] lg:px-4">{saving ? <Loader2 className="h-4 w-4 animate-spin" /> : <Save className="h-4 w-4" />}<span className="hidden sm:inline">Enregistrer</span></button>
    </header>
    <div className="flex min-h-0 flex-1">
      ...
      <main className="relative flex min-w-0 flex-1 flex-col bg-[#302d2a]">
        <div className="absolute left-3 top-3 z-10 flex items-center gap-2"><button type="button" onClick={() => setCompare((value) => !value)} className={`min-h-9 rounded-full px-3 text-[11px] font-black shadow backdrop-blur ${compare ? 'bg-[#e85d00] text-white' : 'bg-black/55 text-white lg:bg-white/90 lg:text-[#231f1b]'}`}>Avant / Après</button>{draftRestored ? <span className="hidden rounded-full bg-white/90 px-3 py-2 text-[11px] font-bold sm:inline"><Cloud className="mr-1 inline h-3.5 w-3.5 text-emerald-600" />Brouillon restauré</span> : null}</div>
        <div className="flex min-h-0 flex-1 items-center justify-center overflow-hidden p-3 pb-20 sm:p-8">
          <div className="relative max-h-full max-w-full overflow-hidden rounded-md bg-[repeating-conic-gradient(#ddd_0_25%,#fff_0_50%)] bg-[length:20px_20px] shadow-2xl touch-none" onPointerDown={onPointerDown} onPointerMove={onPointerMove} onPointerUp={onPointerEnd} onPointerCancel={onPointerEnd}>
        ...
        <div className="absolute bottom-20 left-1/2 flex -translate-x-1/2 items-center gap-2 rounded-full bg-black/60 p-1.5 text-white backdrop-blur lg:bottom-3"><button type="button" onClick={() => change((prev) => ({ ...prev, zoom: Math.max(.7, prev.zoom - .1) }))} className="flex h-10 w-10 items-center justify-center rounded-full hover:bg-white/10"><ZoomOut className="h-4 w-4" /></button><span className="w-12 text-center text-xs font-black">{Math.round(state.zoom * 100)}%</span><button type="button" onClick={() => change((prev) => ({ ...prev, zoom: Math.min(3, prev.zoom + .1) }))} className="flex h-10 w-10 items-center justify-center rounded-full hover:bg-white/10"><ZoomIn className="h-4 w-4" /></button></div>
        {!mobilePanelOpen ? <div className="absolute inset-x-3 bottom-[8.5rem] z-20 flex justify-center gap-2 lg:hidden"><button type="button" onClick={() => change((prev) => ({ ...prev, aspectRatio: 'free', zoom: 1, pan: { x: 0, y: 0 } }))} className="flex min-h-10 items-center gap-1.5 rounded-full bg-white/95 px-3 text-[11px] font-black shadow-lg"><Maximize2 className="h-4 w-4" />Adapter</button><button type="button" onClick={() => change((prev) => ({ ...prev, rotation: (prev.rotation + 90) % 360 }))} className="flex min-h-10 items-center gap-1.5 rounded-full bg-white/95 px-3 text-[11px] font-black shadow-lg"><RotateCw className="h-4 w-4" />Tourner</button><button type="button" onClick={() => change((prev) => applyPreset(prev, 'Studio'))} className="flex min-h-10 items-center gap-1.5 rounded-full bg-white/95 px-3 text-[11px] font-black shadow-lg"><Sparkles className="h-4 w-4 text-[#e85d00]" />Auto</button></div> : null}
        {!mobilePanelOpen && images.length > 1 ? <div className="absolute inset-x-3 bottom-[12rem] z-20 flex justify-center gap-2 overflow-x-auto py-1 lg:hidden">{images.map((item, index) => <button key={item?.url || item?.name || index} type="button" onClick={() => setSelectedIndex(index)} className={`h-12 w-12 shrink-0 overflow-hidden rounded-xl border-2 bg-white shadow-lg ${selectedIndex === index ? 'border-[#e85d00]' : 'border-white'}`}><img src={resolveSource(item)} alt={`Photo ${index + 1}`} className="h-full w-full object-cover" /></button>)}</div> : null}
      </main>
      <aside className="hidden w-[360px] shrink-0 overflow-y-auto border-l border-[#e2dcd2] bg-[#faf8f5] p-5 lg:block">{renderPanel()}</aside>
    </div>
    <div className="absolute inset-x-0 bottom-0 z-30 bg-white pb-[env(safe-area-inset-bottom)] shadow-[0_-12px_35px_rgba(0,0,0,0.22)] lg:hidden">
      {mobilePanelOpen ? <div className="max-h-[62dvh] overflow-y-auto rounded-t-[28px] bg-[#faf8f5] px-4 pb-7"><div className="sticky top-0 z-10 -mx-4 mb-4 flex items-center gap-3 border-b border-[#e8e1d8] bg-[#faf8f5]/95 px-4 pb-3 pt-2 backdrop-blur"><button type="button" onClick={() => setMobilePanelOpen(false)} className="flex h-10 w-10 items-center justify-center rounded-full bg-white shadow-sm" aria-label="Réduire les outils"><ChevronDown className="h-5 w-5" /></button><div className="min-w-0 flex-1"><span className="block text-sm font-black">{activeToolConfig.label}</span><span className="block text-[11px] text-stone-500">Les modifications sont prévisualisées immédiatement</span></div><select value={applyScope} onChange={(event) => setApplyScope(event.target.value)} className="min-h-10 max-w-28 rounded-xl border border-[#e2dcd2] bg-white px-2 text-[10px] font-bold"><option value="selected">Cette photo</option><option value="all">Toutes</option><option value="product">Produit</option></select></div>{renderPanel()}</div> : null}
    ...
    {message ? <div className="fixed bottom-24 left-1/2 z-50 max-w-[90vw] -translate-x-1/2 rounded-full bg-[#231f1b] px-4 py-3 text-center text-xs font-bold text-white shadow-xl lg:bottom-20">{message}<button type="button" className="ml-3" onClick={() => setMessage('')}><X className="inline h-4 w-4" /></button></div> : null}
  </div>;
}
