...
import { AdjustmentsHorizontalIcon, ArrowDownTrayIcon, ArrowLeftIcon, ArrowPathIcon, ArrowUturnLeftIcon, ArrowUturnRightIcon, ArrowsPointingOutIcon, ArrowsRightLeftIcon, ArrowsUpDownIcon, CheckBadgeIcon, CheckIcon, ChevronDownIcon, ClockIcon, CloudIcon, IdentificationIcon, MagnifyingGlassCircleIcon, MagnifyingGlassMinusIcon, MagnifyingGlassPlusIcon, PaintBrushIcon, PhotoIcon, ScissorsIcon, SparklesIcon, Square3Stack3DIcon, SwatchIcon, ViewColumnsIcon, ViewfinderCircleIcon, XMarkIcon } from '@heroicons/react/24/outline';
...
const TOOLS = [
  ...
  { id: 'adjust', label: 'Réglages', icon: AdjustmentsHorizontalIcon },
  { id: 'filters', label: 'Filtres', icon: SwatchIcon },
  { id: 'background', label: 'Arrière-plan', icon: Square3Stack3DIcon },
  { id: 'ai', label: 'Outils IA', icon: SparklesIcon },
  { id: 'watermark', label: 'Filigrane', icon: IdentificationIcon },
  { id: 'export', label: 'Exporter', icon: ArrowDownTrayIcon }
];

// Mobile rail: task-based tools only. 'auto' is an instant action (no panel).
const MOBILE_TOOLS = [
  { id: 'auto', label: 'Auto', icon: SparklesIcon },
  { id: 'crop', label: 'Cadre', icon: ScissorsIcon },
  { id: 'filters', label: 'Filtres', icon: SwatchIcon },
  { id: 'background', label: 'Fond', icon: Square3Stack3DIcon },
  { id: 'watermark', label: 'Filigrane', icon: IdentificationIcon },
  { id: 'adjust', label: 'Réglages', icon: AdjustmentsHorizontalIcon }
];
...
const AI_TOOLS = [
  { id: 'background-remove', label: 'Retirer le fond', description: 'IA locale et privée — aucune image envoyée au serveur', icon: Square3Stack3DIcon, local: true },
  { id: 'enhance', label: 'Améliorer', description: 'Détails, couleurs et balance', icon: SparklesIcon, local: true },
  { id: 'shadow', label: 'Ombre réaliste', description: 'Douce, studio ou flottante', icon: ViewfinderCircleIcon, local: true },
  { id: 'relight', label: 'Changer la lumière', description: 'Éclairage studio instantané', icon: SparklesIcon, local: true },
  { id: 'object-remove', label: 'Effacer un objet', description: 'Pinceau mains, câble, poussière', icon: PaintBrushIcon },
  { id: 'upscale', label: 'Agrandir', description: 'Double les dimensions de sortie', icon: ArrowsPointingOutIcon, local: true },
  { id: 'smart-crop', label: 'Cadrage intelligent', description: 'Centre automatiquement le produit', icon: MagnifyingGlassCircleIcon, local: true }
];
...
export default function ProductImageStudio({ isOpen, image, images = [], initialIndex = 0, shopName = '', onClose, onSave }) {
  ...
  const renderPanel = () => {
    if (activeTool === 'crop') return <div className="space-y-5">
      ...
      <StudioButton className="w-full" icon={ArrowsPointingOutIcon} active={state.aspectRatio === 'free' && state.zoom === 1} onClick={() => change((prev) => ({ ...prev, aspectRatio: 'free', zoom: 1, pan: { x: 0, y: 0 } }))}>Adapter toute la photo</StudioButton>
      ...
      <div className="grid grid-cols-3 gap-2">
        <StudioButton icon={ArrowUturnRightIcon} onClick={() => change((prev) => ({ ...prev, rotation: (prev.rotation + 90) % 360 }))}>Tourner</StudioButton>
        <StudioButton icon={ArrowsRightLeftIcon} active={state.flipX} onClick={() => change((prev) => ({ ...prev, flipX: !prev.flipX }))}>Miroir H</StudioButton>
        <StudioButton icon={ArrowsUpDownIcon} active={state.flipY} onClick={() => change((prev) => ({ ...prev, flipY: !prev.flipY }))}>Miroir V</StudioButton>
      </div>
  ...
  const renderMobilePanel = () => {
    if (activeTool === 'crop') return <div className="space-y-4">
      ...
      <div className="grid grid-cols-4 gap-2">
        <StudioButton icon={ArrowsPointingOutIcon} onClick={() => change((prev) => ({ ...prev, aspectRatio: 'free', zoom: 1, pan: { x: 0, y: 0 } }))}>Adapter</StudioButton>
        <StudioButton icon={ArrowUturnRightIcon} onClick={() => change((prev) => ({ ...prev, rotation: (prev.rotation + 90) % 360 }))}>Tourner</StudioButton>
        <StudioButton icon={ArrowsRightLeftIcon} active={state.flipX} onClick={() => change((prev) => ({ ...prev, flipX: !prev.flipX }))}>Miroir H</StudioButton>
        <StudioButton icon={ArrowsUpDownIcon} active={state.flipY} onClick={() => change((prev) => ({ ...prev, flipY: !prev.flipY }))}>Miroir V</StudioButton>
      </div>
  ...
  return <div className="fixed inset-0 z-[1000] flex flex-col overflow-hidden bg-[#201f1d] text-[#231f1b] lg:bg-[#f5f2ee]" role="dialog" aria-modal="true" aria-label="Studio photo produit">
    <header className="relative z-40 flex min-h-14 items-center gap-1.5 border-b border-white/10 bg-[#201f1d] px-2 pt-[env(safe-area-inset-top)] text-white lg:min-h-16 lg:gap-2 lg:border-[#e2dcd2] lg:bg-white lg:px-5 lg:text-[#231f1b]">
      <button type="button" onClick={onClose} className="flex h-11 w-11 items-center justify-center rounded-full hover:bg-white/10 lg:hover:bg-[#f5f2ee]" aria-label="Fermer"><ArrowLeftIcon className="h-5 w-5" /></button>
      ...
      <button type="button" onClick={() => dispatch({ type: 'UNDO' })} disabled={!history.past.length} className="flex h-11 w-11 items-center justify-center rounded-full hover:bg-[#f5f2ee] disabled:opacity-30" aria-label="Annuler"><ArrowUturnLeftIcon className="h-5 w-5" /></button>
      <button type="button" onClick={() => dispatch({ type: 'REDO' })} disabled={!history.future.length} className="flex h-11 w-11 items-center justify-center rounded-full hover:bg-[#f5f2ee] disabled:opacity-30" aria-label="Rétablir"><ArrowUturnRightIcon className="h-5 w-5" /></button>
      <button type="button" onClick={handleSave} disabled={!loadedImage || saving} className="hidden min-h-11 items-center gap-2 rounded-xl bg-[#e85d00] px-3 text-sm font-black text-white shadow-sm disabled:opacity-50 lg:flex lg:rounded-full lg:bg-[#231f1b] lg:px-4">{saving ? <ArrowPathIcon className="h-4 w-4 animate-spin" /> : <CheckIcon className="h-4 w-4" />}<span className="hidden sm:inline">Enregistrer</span></button>
    </header>
    <div className="flex min-h-0 flex-1">
      ...
      <div className="relative flex min-w-0 flex-1 flex-col bg-[#302d2a]">
        <div className="absolute left-3 top-3 z-10 flex items-center gap-2"><button type="button" onClick={() => setCompare((value) => !value)} className={`min-h-9 rounded-full px-3 text-[11px] font-black shadow ${compare ? 'bg-[#e85d00] text-white' : 'bg-black/55 text-white lg:bg-white/90 lg:text-[#231f1b]'}`}>Avant / Après</button>{draftRestored ? <span className="hidden rounded-full bg-white/90 px-3 py-2 text-[11px] font-bold sm:inline"><CloudIcon className="mr-1 inline h-3.5 w-3.5 text-emerald-600" />Brouillon restauré</span> : null}</div>
        {quality.score > 0 && quality.score < 60 && quality.suggestions?.length ? <button type="button" onClick={() => { setActiveTool('adjust'); setMobilePanelOpen(true); }} className="absolute left-3 top-16 z-10 flex max-w-[80vw] items-center gap-1.5 rounded-full bg-amber-500 px-3 py-2 text-left text-[11px] font-black text-white shadow lg:hidden"><AdjustmentsHorizontalIcon className="h-4 w-4 shrink-0" />{quality.suggestions[0]}</button> : null}
        <div className="flex min-h-0 flex-1 items-center justify-center overflow-hidden p-3 sm:p-8">
          <div className="relative max-h-full max-w-full overflow-hidden rounded-md bg-[repeating-conic-gradient(#ddd_0_25%,#fff_0_50%)] bg-[length:20px_20px] shadow-sm touch-none" onPointerDown={onPointerDown} onPointerMove={onPointerMove} onPointerUp={onPointerEnd} onPointerCancel={onPointerEnd}>
            ...
            {compare ? <div className="pointer-events-none absolute inset-y-0 left-1/2 w-0.5 bg-white shadow"><span className="absolute left-1/2 top-1/2 flex h-9 w-9 -translate-x-1/2 -translate-y-1/2 items-center justify-center rounded-full bg-white shadow"><ViewColumnsIcon className="h-4 w-4" /></span></div> : null}
          </div>
        </div>
        <div className="absolute bottom-3 left-1/2 flex -translate-x-1/2 items-center gap-2 rounded-full bg-black/60 p-1.5 text-white"><button type="button" onClick={() => change((prev) => ({ ...prev, zoom: Math.max(.3, prev.zoom - .1) }))} className="flex h-10 w-10 items-center justify-center rounded-full hover:bg-white/10"><MagnifyingGlassMinusIcon className="h-4 w-4" /></button><span className="w-12 text-center text-xs font-black">{Math.round(state.zoom * 100)}%</span><button type="button" onClick={() => change((prev) => ({ ...prev, zoom: Math.min(3, prev.zoom + .1) }))} className="flex h-10 w-10 items-center justify-center rounded-full hover:bg-white/10"><MagnifyingGlassPlusIcon className="h-4 w-4" /></button></div>
    ...
    <div className="border-t border-[#e8e1d8] bg-white pb-[env(safe-area-inset-bottom)] shadow-sm lg:hidden">
      {mobilePanelOpen && activeMobileTool ? <div className="max-h-[45dvh] overflow-y-auto bg-[#faf8f5] px-4 pb-4 pt-3">
        <div className="mb-3 flex items-center justify-between">
          ...
          <button type="button" onClick={() => setMobilePanelOpen(false)} className="flex h-9 w-9 items-center justify-center rounded-full bg-white shadow-sm" aria-label="Réduire les outils"><ChevronDownIcon className="h-5 w-5" /></button>
        </div>
        {renderMobilePanel()}
      </div> : null}
      ...
      <div className="border-t border-[#ebe5dd] px-3 pb-2 pt-2">
        ...
        <button type="button" onClick={handleSave} disabled={!loadedImage || saving} className="flex min-h-12 w-full items-center justify-center gap-2 rounded-2xl bg-[#e85d00] px-4 text-sm font-black text-white shadow-sm transition active:bg-[#c94e00] disabled:cursor-not-allowed disabled:opacity-50">{saving ? <ArrowPathIcon className="h-5 w-5 animate-spin" /> : <CheckIcon className="h-5 w-5" />}{saving ? 'Enregistrement…' : images.length > 1 && applyScope === 'all' ? 'Enregistrer toutes les photos' : 'Enregistrer la photo'}</button>
    ...
    {message ? <div className="fixed left-1/2 top-[calc(env(safe-area-inset-top)+4.5rem)] z-50 max-w-[90vw] -translate-x-1/2 rounded-full bg-[#231f1b] px-4 py-3 text-center text-xs font-bold text-white shadow-sm lg:bottom-20 lg:top-auto">{message}<button type="button" className="ml-3" onClick={() => setMessage('')}><XMarkIcon className="inline h-4 w-4" /></button></div> : null}
  </div>;
}
